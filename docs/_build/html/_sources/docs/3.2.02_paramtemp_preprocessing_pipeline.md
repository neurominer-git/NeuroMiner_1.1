...
:::{important}
A critical point to remember in this mode is that if the user is interested in observing the Z-score by using [**visualization**](3.6_mainmenu_visualize_classifiers), it is essential for the user to use smoothed data first. Applying a scaling model to unsmoothed imaging data will likely result in extremely high/low values in the test data due to non-overlap of tissue borders between training and test cases.
:::

## Regress out nuisance covariates

If covariates have been entered alongside the [data](3.1_mainmenu_input_data), you can apply covariate correction using this step. This option is designed to remove the variance associated with a nuisance variable (e.g., age, sex, study center) from the data within each CV fold. When chosen it will reveal the following options:
...
:::{note}
This function will perform winsorization (i.e., clamping) because values the stop value of alpha (e.g., above 4 standard deviations above or below the mean) will be given the final value.
:::
...
:::{note}
Wrappers and will further be discussed in [ensemble generation straqtegies](3.2.05_ensemble_generation_strategies).
:::
